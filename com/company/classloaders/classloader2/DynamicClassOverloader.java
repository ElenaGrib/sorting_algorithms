...
public class DynamicClassOverloader extends ClassLoader {
    ...
    protected Class findClass(String name) throws ClassNotFoundException {
        ...
        if (f == null) {
            /**
             * Обращаемся к системному загрузчику в случае неудачи. findSystemClass – это метод
             абстрактного класса ClassLoader с объявлением protected Class findSystemClass(String name)
             (т.е. предназначенный для использования в наследниках и не подлежащий переопределению).
             Он выполняет поиск и загрузку класса по алгоритму системного загрузчика. Без вызова
             findSystemClass(name) нам пришлось бы самостоятельно позаботиться о загрузке всех
             стандартных библиотечных классов типа java.lang.String, что потребовало бы
             реализовать работу с JAR-архивами (стандартные библиотеки почти всегда
             упакованы в JAR)*/
    ...
    public static byte[] loadFileAsBytes(File file) throws IOException {
        ...
        try {
            ...
            try {
                ...
                /** Игнорируем исключения, возникшие при вызове close. Они крайне маловероятны и не
                 очень важны - файл уже прочитан. Но если они все же возникнут, то они не должны
                 замаскировать действительно важные ошибки, возникшие при вызове read.*/
            }
        }
        return result;
    }
